...
export const CartProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    try {
      ...
      // Correction : suppression du (e) inutilisé
      console.warn("Impossible de charger le panier depuis le localStorage");
    }
  }, []);
  ...
  const subtotal = useMemo(() => 
    cart.reduce((acc, item) => acc + (item.prix * item.quantity), 0), [cart]
  );
